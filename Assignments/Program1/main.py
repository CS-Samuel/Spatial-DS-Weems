            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for k,v in gd.adjusted_poly_dict.items():
                    for i in range(len(v)):
                        if point_inside_polygon(x,y,v[i]):
                            for j in range(len(v)):
                                left_right_Boundaries = []
                                upper_lower_Boundaries = []
                                pygame.draw.polygon(screen, black, v[j], 5)
                                for p in v[j]:
                                    x,y = p
                                    left_right_Boundaries.append(x)
                                    upper_lower_Boundaries.append(y)
                                pygame.draw.rect(screen, black, (min(left_right_Boundaries), min(upper_lower_Boundaries), (max(left_right_Boundaries)-min(left_right_Boundaries)), (max(upper_lower_Boundaries)-min(upper_lower_Boundaries))), 5)    
