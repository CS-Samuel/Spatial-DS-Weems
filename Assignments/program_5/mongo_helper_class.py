...
class mongoHelper(object):
    def __init__(self):
        ...
        self.db_meteors = self.client.world_data.meteorites

    def get_airport_coordinates(self,code):
        """
        
        Get the coordinates for an airport by airport code

        """    

        airport = self.db_airports.find_one( {"properties.ap_iata" : code})
        return (airport['geometry']['coordinates'])
    
    def get_airport_continent(self,code):
        """
        
        Get the continent for an airport by airport code

        """  

        airport = self.db_airports.find_one({"properties.ap_iata": code})
        return (airport['properties']['continent'])
    ...
    def get_nearest_airports(self,lon,lat,r):
        airports = self.db_airports.find( { 'geometry': { '$geoWithin': { '$centerSphere': [ [lon, lat ] , r / 3963.2 ] } }} )
        coordinates = []
       
        for coord in airports:
