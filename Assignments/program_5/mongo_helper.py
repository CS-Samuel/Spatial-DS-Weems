...
class MongoHelper(object):
    ...
    def get_features_limited_near_me(self, collection, field, min_max, min_max_value, point, radius, earth_radius=3963.2):
        ...
        if (min_max == "min"):
            res = self.client['world_data'][collection].find( {'geometry' : { '$geoWithin': { '$centerSphere': [[x,y], float(radius)/earth_radius]}}, properties_field: {'$gt' : float(min_max_value)}})
        if (min_max == "max"):
            res = self.client['world_data'][collection].find( {'geometry' : { '$geoWithin': { '$centerSphere': [[x,y], float(radius)/earth_radius]}}, properties_field: {'$lt' : float(min_max_value)}})

        return self._make_result_list(res)

#db.meteorites.find({ "properties.mass" : { $gt :  '5000', $lt : '7000'}});
    ...
    def _make_result_list(self,res):
        ...
        res_list = []
        for r in res:
            res_list.append(r['geometry']['coordinates'])
